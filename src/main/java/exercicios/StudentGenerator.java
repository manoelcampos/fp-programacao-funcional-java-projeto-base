...
public class StudentGenerator {
     ...
     * criar um aluno.
     */
    private static final String[] HOMENS_ARRAY = HOMENS.split(";");

    /**
     * Vetor contendo uma lista de nomes feminimos a serem sorteados aleatoriamente para
     * criar um aluno.
     */
    private static final String[] MULHERES_ARRAY = MULHERES.split(";");
    ...
    public final Estado[] ESTADOS = {
            new Estado("Tocantins", "TO"),
            new Estado("Paraná", "PR"),
            new Estado("São Paulo", "SP"),
    };

    public final Cidade[] CIDADES = {
            new Cidade("Palmas", ESTADOS[0]),
            new Cidade("Porto Nacional", ESTADOS[0]),
            new Cidade("Paraíso do Tocantins", ESTADOS[0]),
            new Cidade("Curitiba", ESTADOS[1]),
            new Cidade("São Paulo", ESTADOS[2]),
            new Cidade("Santos", ESTADOS[2]),
    };

    public final Curso[] CURSOS;
     ...
     * Irá usar data/hora atual como seed (semente) do gerador de números aleatórios.
     * Assim, irá gerar estudantes aleatórios diferentes.
     * @see #StudentGenerator(long)
     * @see #generate(int) */
     ...
     * @see #generate(int) */
    public StudentGenerator(final long seed) {
        this.rand = new Random(seed);
        CURSOS = new Curso[]{
                new Curso(1, "Sistemas para Internet", 6, randCampus()),
                new Curso(2, "Engenharia Elétrica", 10, randCampus()),
                new Curso(3, "Engenharia Civil", 10, randCampus()),
                new Curso(4, "Matemática", 8, randCampus()),
                new Curso(5, "Educação Física", 8, randCampus()),
                new Curso(6, "Gestão Pública", 6, randCampus())
        };
    }
    ...
    public List<Estudante> generate(final int total){
        ...
        /* Não usa toList() pois deseja-se uma lista mutável, apenas para dar flexibilidade nos exemplos,
         para que o aluno possa fazer o que bem entender com a lista e experimentar diferentes operações. */
        return IntStream.rangeClosed(1, total).mapToObj(this::randStudent).collect(toCollection(() -> new ArrayList<>(total)));
    }
    ...
    public Estudante randStudent(final int id) {
        ...
        return new Estudante(
                ...
                randGradYear(), randCourse(CURSOS, true),
                randCidade(CIDADES)
        );
    }
     ...
     * @return
     */
    private String randName(char gender){
        gender = Character.toUpperCase(gender);
        return gender == 'M' ? HOMENS_ARRAY[randInt(HOMENS_ARRAY.length)] : MULHERES_ARRAY[randInt(MULHERES_ARRAY.length)];
    }
     ...
     * @return
     */
    private double randDouble(final double max){
